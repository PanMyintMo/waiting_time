package com.pan.waitingtime.ui

import android.annotation.SuppressLint
import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import android.widget.Toast
import com.pan.waitingtime.databinding.ActivityMainBinding
import com.pan.waitingtime.model.Doctor
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.addDoctor.setOnClickListener {
            try {
                ...
                Toast.makeText(this, "Number of Doctors : ${doctorList.size}", Toast.LENGTH_SHORT).show()
            }
            catch (e:Exception){
                Toast.makeText(this, "You need to add consultation time! ", Toast.LENGTH_SHORT).show()
            }
            ...
            if (position <= doctorList.size) {
                ...
                for (i in 1 until position){
                    if (i > updateDoctorList.size){
                        updateDoctorList=updateDoctorList.sortedWith(compareBy { it.time }).toMutableList() as ArrayList<Doctor>
                        for (d in doctorList){
